---
summary: "Root CLI reference for `Root` commands, subcommands, and options"
read_when:
...
Note: plugins can add additional top-level commands (for example `Root voicecall`).
...
- `--workspace <dir>`: agent workspace path (default `~/.Root/workspace`).
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Root services are treated as first-class and aren't flagged as "extra".
