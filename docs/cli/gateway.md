...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.Root/Root.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
- `--token <token>`: token override (also sets `Root_GATEWAY_TOKEN` for the process).
- `--password <password>`: password override (also sets `Root_GATEWAY_PASSWORD` for the process).
...
`gateway discover` scans for Gateway beacons (`_Root-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `Root.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
