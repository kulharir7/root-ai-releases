...
Root 运行一个源自 **pi-mono** 的嵌入式智能体运行时。
...
在 `agents.defaults.workspace` 内，Root 期望以下用户可编辑的文件：
...
在新会话的第一轮，Root 将这些文件的内容直接注入智能体上下文。
...
Root 从三个位置加载 Skills（名称冲突时工作区优先）：
...
Root 复用 pi-mono 代码库的部分内容（模型/工具），但**会话管理、设备发现和工具连接由 Root 负责**。
