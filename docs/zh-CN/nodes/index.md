...
- **批准**：通过 `~/.Root/exec-approvals.json` 在节点主机上执行。
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.Root/Root.json`）。
- `Root node run` 读取 `Root_GATEWAY_TOKEN` 进行认证。
...
批准存储在节点主机的 `~/.Root/exec-approvals.json` 中。
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.Root/exec-approvals.json`）。
...
Root 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
...
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.Root/node.json` 中。
- Exec 批准通过 `~/.Root/exec-approvals.json` 在本地执行
  （参见 [Exec 批准](/tools/exec-approvals)）。
- 在 macOS 上，当配套应用 exec 主机可达时，无头节点主机优先使用它，
  如果应用不可用则回退到本地执行。设置 `Root_NODE_EXEC_HOST=app` 要求
  使用应用，或设置 `Root_NODE_EXEC_FALLBACK=0` 禁用回退。
