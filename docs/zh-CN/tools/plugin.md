...
插件只是一个**小型代码模块**，用额外功能（命令、工具和 Gateway 网关 RPC）扩展 Root。

大多数时候，当你想要一个尚未内置到核心 Root 的功能（或你想将可选功能排除在主安装之外）时，你会使用插件。
...
- 从 2026.1.15 起 Microsoft Teams 仅作为插件提供；如果使用 Teams，请安装 `@Root/msteams`。
...
Root 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/plugins/manifest)。
...
插件目录可以包含带有 `Root.extensions` 的 `package.json`：
...
渠道插件可以通过 `Root.channel` 广播新手引导元数据，通过 `Root.install` 广播安装提示。这使核心目录保持无数据。
...
Root 还可以合并**外部渠道目录**（例如，MPM 注册表导出）。将 JSON 文件放在以下位置之一：

- `~/.Root/mpm/plugins.json`
- `~/.Root/mpm/catalog.json`
- `~/.Root/plugins/catalog.json`

或将 `Root_PLUGIN_CATALOG_PATHS`（或 `Root_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（逗号/分号/`PATH` 分隔）。每个文件应包含 `{ "entries": [ { "name": "@scope/pkg", "Root": { "channel": {...}, "install": {...} } } ] }`。
...
如果插件导出 `id`，Root 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
- 插件配置使用嵌入在 `Root.plugin.json`（`configSchema`）中的 JSON Schema 进行验证。
...
插件也可以注册自己的顶级命令（例如：`Root voicecall`）。
...
插件可以注册**模型提供商认证**流程，以便用户可以在 Root 内运行 OAuth 或 API 密钥设置（无需外部脚本）。
...
- 主包：`Root`（本仓库）
- 插件：`@Root/*` 下的独立 npm 包（例如：`@Root/voice-call`）
...
- 单独发布的插件应运行自己的 CI（lint/构建/测试）并验证 `Root.extensions` 指向构建的入口点（`dist/index.js`）。
