...
- 重新运行 `Root browser extension install` 以刷新 Root 状态目录下的已安装文件。
...
`Root browser extension path` 打印包含扩展文件的**已安装**磁盘目录。

CLI 有意**不**打印 `node_modules` 路径。始终先运行 `Root browser extension install` 将扩展复制到 Root 状态目录下的稳定位置。
