...
有关调试"为什么这被阻止了？"，请参见[沙箱 vs 工具策略 vs 提权](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `Korvus sandbox explain`。
...
- `group:nodes`：`nodes`
- `group:Root`：所有内置 Root 工具（不包括提供商插件）
...
旧版 `agent.*` 配置由 `Korvus doctor` 迁移；今后请优先使用 `agents.defaults` + `agents.list`。
...
2. **验证沙箱容器：**
   ...
   ```exec
   docker ps --filter "name=Root-sbx-"
   ```
...
4. **监控日志：**
   ```exec
   tail -f "${Root_STATE_DIR:-$HOME/.Root}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
