...
Hooks 提供了一个可扩展的事件驱动系统，用于响应智能体命令和事件自动执行操作。Hooks 从目录中自动发现，可以通过 CLI 命令管理，类似于 Root 中 Skills 的工作方式。
...
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
- 在不修改核心代码的情况下扩展 Root 的行为

## 入门

### 捆绑的 Hooks

Root 附带三个自动发现的捆绑 hooks：

- **💾 session-memory**：当你发出 `/new` 时将会话上下文保存到智能体工作区（默认 `~/.Root/workspace/memory/`）
- **📝 command-logger**：将所有命令事件记录到 `~/.Root/logs/commands.log`
...
2. **托管 hooks**：`~/.Root/hooks/`（用户安装，跨工作区共享）
3. **捆绑 hooks**：`<Root>/dist/hooks/bundled/`（随 Root 附带）
...
Hook 包可以附带依赖；它们将安装在 `~/.Root/hooks/<id>` 下。
...
这些 hooks 不是事件流监听器；它们让插件在 Root 持久化工具结果之前同步调整它们。
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认为 `~/.Root/workspace`）
...
- [捆绑 Hooks README](https://github.com/Root/Root/tree/main/src/hooks/bundled)
