...
read_when:
  - 更新 Root
  - 更新后出现问题
summary: 安全更新 Root（全局安装或源码），以及回滚策略
title: 更新
x-i18n:
...
- 对于**全局安装**，脚本底层使用 `npm install -g Root@latest`。
...
- 检测并将旧版 Gateway 网关服务（launchd/systemd；旧版 schtasks）迁移到当前 Root 服务。
...
提示：要查看当前发布的版本，运行 `npm view Root version`。
