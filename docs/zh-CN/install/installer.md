...
Root 提供两个安装器脚本（托管在 `Root.ai`）：

- `https://Root.ai/install.sh` — "推荐"安装器（默认全局 npm 安装；也可从 GitHub 检出安装）
- `https://Root.ai/install-cli.sh` — 无需 root 权限的 CLI 安装器（安装到带有独立 Node 的前缀目录）
- `https://Root.ai/install.ps1` — Windows PowerShell 安装器（默认 npm；可选 git 安装）

查看当前参数/行为，运行：

```bash
curl -fsSL https://Root.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) 帮助：

```powershell
& ([scriptblock]::Create((iwr -useb https://Root.ai/install.ps1))) -?
```

如果安装器完成但在新终端中找不到 `Root`，通常是 Node/npm PATH 问题。参见：[安装](/install#nodejs--npm-path-sanity)。
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://Root.ai/install.sh | bash
```

### 可发现性 / "git 安装"提示

如果你在**已有的 Root 源码检出目录中**运行安装器（通过 `package.json` + `pnpm-workspace.yaml` 检测），它会提示：

- 更新并使用此检出（`git`）
- 或迁移到全局 npm 安装（`npm`）

在非交互式上下文中（无 TTY / `--no-prompt`），你必须传入 `--install-method git|npm`（或设置 `Root_INSTALL_METHOD`），否则脚本将以退出码 `2` 退出。
...
此脚本将 `Root` 安装到前缀目录（默认：`~/.Root`），同时在该前缀下安装专用的 Node 运行时，因此可以在不想改动系统 Node/npm 的机器上使用。

帮助：

```bash
curl -fsSL https://Root.ai/install-cli.sh | bash -s -- --help
```
...
```powershell
iwr -useb https://Root.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://Root.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\Root"
```

环境变量：

- `Root_INSTALL_METHOD=git|npm`
- `Root_GIT_DIR=...`
