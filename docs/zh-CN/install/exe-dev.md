...
summary: 在 exe.dev 上运行 Root Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：Root Gateway 网关运行在 exe.dev VM 上，可从你的笔记本电脑通过以下地址访问：`https://<vm-name>.exe.xyz`

本页假设使用 exe.dev 的默认 **exeuntu** 镜像。如果你选择了不同的发行版，请相应地映射软件包。

## 新手快速路径

1. [https://exe.new/Root](https://exe.new/Root)
2. 根据需要填写你的认证密钥/令牌
...
提示：保持此 VM **有状态**。Root 在 `~/.Root/` 和 `~/.Root/workspace/` 下存储状态。
...
访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`（参阅新手引导中的控制 UI 输出）。使用 `Root devices list` 和 `Root devices approve <requestId>` 批准设备。如有疑问，从浏览器使用 Shelley！
