...
summary: 设置指南：在保持最新的同时保持你的 Root 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.Root/workspace`（工作区）+ `~/.Root/Root.json`（配置）。
...
- **配置：** `~/.Root/Root.json`（JSON/JSON5 格式）
- **工作区：** `~/.Root/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
- **配对允许列表**：`~/.Root/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.Root/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.Root/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.Root/workspace` 和 `~/.Root/` 保持为"你的东西"；不要将个人提示/配置放入 `Root` 仓库。
