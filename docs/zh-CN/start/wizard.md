...
新手引导向导是在 macOS、Linux 或 Windows（通过 WSL2；强烈推荐）上设置 Root 的**推荐**方式。
...
2. **模型/认证**
   ...
   - **OpenAI API 密钥**：如果存在则使用 `OPENAI_API_KEY`，否则提示输入密钥，然后保存到 `~/.Root/.env` 以便 launchd 可以读取。
...
- OAuth 凭证存储在 `~/.Root/credentials/oauth.json`；认证配置文件存储在 `~/.Root/agents/<agentId>/agent/auth-profiles.json`（API 密钥 + OAuth）。
- 更多详情：[/concepts/oauth](/concepts/oauth)

3. **工作区**
   - 默认 `~/.Root/workspace`（可配置）。
   - 为智能体引导仪式播种所需的工作区文件。
...
5. **渠道**
   ...
   - 私信安全：默认为配对。第一条私信发送验证码；通过 `Root pairing approve <channel> <code>` 批准或使用允许列表。
...
- 默认工作区遵循 `~/.Root/workspace-<agentId>`。
...
- 下载适当的发布资源。
- 存储在 `~/.Root/tools/signal-cli/<version>/` 下。
