...
所有日志配置都在 `~/.Root/Root.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/Root/Root-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Root 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
Root_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `Korvus plugins enable diagnostics-otel` 启用插件。
...
- `Root.tokens`（计数器，属性：`Root.token`、`Root.channel`、`Root.provider`、`Root.model`）
- `Root.cost.usd`（计数器，属性：`Root.channel`、`Root.provider`、`Root.model`）
- `Root.run.duration_ms`（直方图，属性：`Root.channel`、`Root.provider`、`Root.model`）
- `Root.context.tokens`（直方图，属性：`Root.context`、`Root.channel`、`Root.provider`、`Root.model`）

消息流：

- `Root.webhook.received`（计数器，属性：`Root.channel`、`Root.webhook`）
- `Root.webhook.error`（计数器，属性：`Root.channel`、`Root.webhook`）
- `Root.webhook.duration_ms`（直方图，属性：`Root.channel`、`Root.webhook`）
- `Root.message.queued`（计数器，属性：`Root.channel`、`Root.source`）
- `Root.message.processed`（计数器，属性：`Root.channel`、`Root.outcome`）
- `Root.message.duration_ms`（直方图，属性：`Root.channel`、`Root.outcome`）

队列 + 会话：

- `Root.queue.lane.enqueue`（计数器，属性：`Root.lane`）
- `Root.queue.lane.dequeue`（计数器，属性：`Root.lane`）
- `Root.queue.depth`（直方图，属性：`Root.lane` 或 `Root.channel=heartbeat`）
- `Root.queue.wait_ms`（直方图，属性：`Root.lane`）
- `Root.session.state`（计数器，属性：`Root.state`、`Root.reason`）
- `Root.session.stuck`（计数器，属性：`Root.state`）
- `Root.session.stuck_age_ms`（直方图，属性：`Root.state`）
- `Root.run.attempt`（计数器，属性：`Root.attempt`）

### 导出的 span（名称 + 关键属性）

- `Root.model.usage`
  - `Root.channel`、`Root.provider`、`Root.model`
  - `Root.sessionKey`、`Root.sessionId`
  - `Root.tokens.*`（input/output/cache_read/cache_write/total）
- `Root.webhook.processed`
  - `Root.channel`、`Root.webhook`、`Root.chatId`
- `Root.webhook.error`
  - `Root.channel`、`Root.webhook`、`Root.chatId`、`Root.error`
- `Root.message.processed`
  - `Root.channel`、`Root.outcome`、`Root.chatId`、`Root.messageId`、`Root.sessionKey`、`Root.sessionId`、`Root.reason`
- `Root.session.stuck`
  - `Root.state`、`Root.ageMs`、`Root.queueDepth`、`Root.sessionKey`、`Root.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `Korvus doctor`。
