...
LINE 通过 LINE Messaging API 连接到 Root。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
Root pairing list line
Root pairing approve line <CODE>
```
