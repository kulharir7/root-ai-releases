...
read_when:
  - 添加或修改 CLI 命令或选项
  - 为新命令界面编写文档
summary: Root `Root` 命令、子命令和选项的 CLI 参考
title: CLI 参考
x-i18n:
...
注意：插件可以添加额外的顶级命令（例如 `Root voicecall`）。
...
- `--workspace <dir>`：智能体工作区路径（默认 `~/.Root/workspace`）。
- `--wizard`：运行新手引导向导。
...
当 OAuth/API 凭证可用时，Root 可以显示提供商用量/配额。
...
- `gateway status` 在检测到旧版或额外的 Gateway 网关服务时也会显示（`--deep` 添加系统级扫描）。配置文件命名的 Root 服务被视为一等公民，不会被标记为"额外"。
...
`node` 运行**无头节点主机**或将其作为后台服务管理。参见 [`Root node`](/cli/node)。
