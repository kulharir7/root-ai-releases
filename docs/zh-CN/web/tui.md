...
```bash
Root tui --url ws://<host>:<port> --token <gateway-token>
```
...
- 确认智能体可以运行：`Korvus status` 和 `Korvus models status`。
...
- 选择器中没有智能体：检查 `Korvus agents list` 和你的路由配置。
