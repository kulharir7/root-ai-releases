...
变量如 `DISPLAY` 应该放在 `~/.Root/.env` 中（由 Gateway 网关早期加载）。
...
症状：`pwd` 或文件工具显示 `~/.Root/sandboxes/...` 即使你
期望的是主机工作区。
...
Root 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${Root_CONFIG_PATH:-$HOME/.Root/Root.json}"
```
...
```bash
ls -la ~/.Root/agents/<agentId>/sessions/
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/Root/Root-*.log | head -1)" | tail -20
```
...
当配置包含未知键、格式错误的值或无效类型时，Root 现在拒绝启动。
这是为了安全而故意设计的。
...
```bash
curl -fsSL https://Root.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
端点接受 JSON Schema 的严格子集。Root 在当前 `main` 中清理/规范化工具
...
如果重置不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/Root/Root/blob/main/scripts/package-mac-app.sh) 中更改 `BUNDLE_ID`（例如，添加 `.test` 后缀）并重新构建。这会强制 macOS 将其视为新应用。
...
**修复 3：检查 CLI 安装**
确保全局 `Root` CLI 已安装且与应用版本匹配：

```bash
Root --version
npm install -g Root@<version>
```
...
| 日志                             | 位置                                                                                                                                                                                                                                                                                                                      |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway 网关文件日志（结构化）   | `/tmp/Root/Root-YYYY-MM-DD.log`（或 `logging.file`）                                                                                                                                                                                                                                                              |
| Gateway 网关服务日志（监管程序） | macOS：`$Root_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.Root/logs/...`；配置文件使用 `~/.Root-<profile>/logs/...`）<br />Linux：`journalctl --user -u Root-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "Root Gateway (<profile>)" /V /FO LIST` |
| 会话文件                         | `$Root_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                          |
| 媒体缓存                         | `$Root_STATE_DIR/media/`                                                                                                                                                                                                                                                                                              |
| 凭证                             | `$Root_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                        |
...
1. 首先检查日志：`/tmp/Root/`（默认：`Root-YYYY-MM-DD.log`，或你配置的 `logging.file`）
2. 在 GitHub 上搜索现有问题
3. 提交新问题时包含：
   - Root 版本
   - 相关日志片段
