...
Root 的 Gateway 网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
- 当 `gateway.auth.mode="token"` 时，使用 `gateway.auth.token`（或 `Root_GATEWAY_TOKEN`）。
- 当 `gateway.auth.mode="password"` 时，使用 `gateway.auth.password`（或 `Root_GATEWAY_PASSWORD`）。

## 选择智能体

无需自定义头：在 OpenResponses `model` 字段中编码智能体 id：

- `model: "Root:<agentId>"`（示例：`"Root:main"`、`"Root:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过头指定特定的 Root 智能体：

- `x-Root-agent-id: <agentId>`（默认：`main`）

高级：

- `x-Root-session-key: <sessionKey>` 完全控制会话路由。
