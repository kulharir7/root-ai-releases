...
- 你可以在 Gateway 网关机器上运行 CLI（`Root`）（或通过 SSH）。
...
- 在 Gateway 网关主机的 `~/.Root/Root.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `Root.internal.`）并发布 `_Root-gw._tcp` 记录。
...
1. 在 Gateway 网关主机上创建 `~/.Root/workspace/canvas/index.html`。
...
A2UI 主机位于 `http://<gateway-host>:18793/__Root__/a2ui/`。
