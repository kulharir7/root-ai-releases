---
read_when:
  - 在 Oracle Cloud 上设置 Root
  - 寻找 Root 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 Root
summary: 在 Oracle Cloud 上运行 Root（Always Free ARM）
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud（OCI）上运行 Root

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层上运行持久化的 Root Gateway 网关。

Oracle 的免费层非常适合 Root（特别是如果你已经有 OCI 账户），但有一些权衡：
...
sudo tailscale up --ssh --hostname=Root
```

这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh Root` 连接——不需要公网 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@Root`（或使用 Tailscale IP）。
...
# 检查守护进程状态
systemctl --user status Root-gateway
...
```
https://Root.<tailnet-name>.ts.net/
```
...
ssh -L 18789:127.0.0.1:18789 ubuntu@Root
```
...
# 重新认证
sudo tailscale up --ssh --hostname=Root --reset
```
...
- `~/.Root/` — 配置、凭证、会话数据
- `~/.Root/workspace/` — 工作区（SOUL.md、记忆、产物）

定期备份：

```bash
tar -czvf Root-backup.tar.gz ~/.Root ~/.Root/workspace
```
