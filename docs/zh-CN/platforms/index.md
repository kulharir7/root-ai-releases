...
Root 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- macOS：LaunchAgent（`bot.molt.gateway` 或 `bot.molt.<profile>`；旧版 `com.Root.*`）
- Linux/WSL2：systemd 用户服务（`Root-gateway[-<profile>].service`）
