...
summary: Root macOS 配套应用（菜单栏 + Gateway 网关代理）
title: macOS 应用
x-i18n:
...
- 根据请求通过 npm/pnpm 安装全局 CLI（`Root`）（不建议使用 bun 作为 Gateway 网关运行时）。
...
应用管理一个标记为 `bot.molt.gateway` 的每用户 LaunchAgent（使用 `--profile`/`Root_PROFILE` 时为 `bot.molt.<profile>`；旧版 `com.Root.*` 仍会卸载）。
