...
read_when:
  - 启动新的 Root 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 Root 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — Root 个人助手（默认）

## 首次运行（推荐）

Root 为智能体使用专用的工作区目录。默认：`~/.Root/workspace`（可通过 `agents.defaults.workspace` 配置）。
