...
- Publish as `Root/plugin-sdk` (or export from core under `Root/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `RootPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: RootConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: RootConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: RootConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: RootConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: RootConfig): string;
  };
};
```
...
- Introduce `Root/plugin-sdk`.
- Add `api.runtime` to `RootPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `RootRuntime: ">=2026.2.0"`).
