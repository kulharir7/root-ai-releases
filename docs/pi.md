...
This document describes how Root integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.
...
| `pi-tui`          | Terminal UI components (used in Root's local TUI mode)                                             |
...
├── pi-tools.ts                    # createRootCodingTools()
...
2. **Custom Replacements**: Root replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Root Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Root's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Root CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Root wraps this with `guardSessionManager()` for tool result safety.
...
Root maintains an auth profile store with multiple API keys per provider:
...
Root loads custom pi extensions for specialized behavior:
...
Root also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Root Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Root tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.Root/agents/<agentId>/sessions/` (or `$Root_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-Root-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-Root-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-Root-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-Root-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
