...
Root treats **one direct-chat session per agent** as primary. Direct chats collapse to `agent:<agentId>:<mainKey>` (default `main`), while group/channel chats get their own keys. `session.mainKey` is honored.
...
All session state is **owned by the gateway** (the “master” Root). UI clients (macOS app, WebChat, etc.) must query the gateway for session lists and token counts instead of reading local files.
...
- On the **gateway host**:
  - Store file: `~/.Root/agents/<agentId>/sessions/sessions.json` (per agent).
- Transcripts: `~/.Root/agents/<agentId>/sessions/<SessionId>.jsonl` (Telegram topic sessions use `.../<SessionId>-topic-<threadId>.jsonl`).
- The store is a map `sessionKey -> { sessionId, updatedAt, ... }`. Deleting entries is safe; they are recreated on demand.
- Group entries may include `displayName`, `channel`, `subject`, `room`, and `space` to label sessions in UIs.
- Session entries include `origin` metadata (label + routing hints) so UIs can explain where a session came from.
- Root does **not** read legacy Pi/Tau session folders.

## Session pruning

Root trims **old tool results** from the in-memory context right before LLM calls by default.
