...
read_when:
  - Designing workspace memory (~/.Root/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Root integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.Root/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.Root/workspace` as canonical human-readable memory.
...
```
~/.Root/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Root**, but keep a separable core library.

### Why integrate into Root?
...
Pragmatic take for `~/.Root/workspace`:
