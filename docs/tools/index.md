...
Root exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `Root-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `Root.json`
...
- `group:nodes`: `nodes`
- `group:Root`: all built-in Root tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`Root node run`).
...
Control the dedicated Root-managed browser.
