...
- The `Root` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The default `chrome` profile uses the **system default Chromium browser** via the
  extension relay; switch to `Root` for the isolated managed browser.
...
## Profiles: `Root` vs `chrome`

- `Root`: managed, isolated browser (no extension required).
- `chrome`: extension relay to your **system browser** (requires the Root
  extension to be attached to a tab).

Set `browser.defaultProfile: "Root"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.Root/Root.json`.
...
- If you override the Gateway port (`gateway.port` or `Root_GATEWAY_PORT`),
...
- Default profile is `chrome` (extension relay). Use `defaultProfile: "Root"` for the managed browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `Root` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
Root preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Root can
...
- If browser control is enabled and no auth is configured, Root auto-generates `gateway.auth.token` on startup and persists it to config.
...
- The `Root` profile is auto-created if missing.
...
- You click the **Root Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
When launching locally, Root picks the first available:
...
- `x-Root-password: <gateway password>` or HTTP Basic auth with that password
...
`Root_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- Download and trace output paths are constrained to Root temp roots:
  - traces: `/tmp/Root` (fallback: `${os.tmpdir()}/Root`)
  - downloads: `/tmp/Root/downloads` (fallback: `${os.tmpdir()}/Root/downloads`)
- Upload paths are constrained to an Root temp uploads root:
  - uploads: `/tmp/Root/uploads` (fallback: `${os.tmpdir()}/Root/uploads`)
- `upload` can also set file inputs directly via `--input-ref` or `--element`.
- `snapshot`:
  - `--format ai` (default when Playwright is installed): returns an AI snapshot with numeric refs (`aria-ref="<n>"`).
  - `--format aria`: returns the accessibility tree (no refs; inspection only).
  - `--efficient` (or `--mode efficient`): compact role snapshot preset (interactive + compact + depth + lower maxChars).
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/gateway/configuration#browser-Root-managed-browser)).
...
- `browser` accepts:
  - `profile` to choose a named browser profile (Root, chrome, or remote CDP).
