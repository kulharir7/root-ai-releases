---
summary: "Chrome extension: let Root drive your existing Chrome tab"
read_when:
...
The Root Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate Root-managed Chrome profile.
...
Root then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Root release (npm package) as static files. There is no separate “build” step.
...
Root ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- **This is not isolated** like the dedicated Root-managed profile.
