...
into core Root yet (or you want to keep optional features out of your main
install).
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@Root/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@Root/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@Root/zalouser`
- [Matrix](/channels/matrix) — `@Root/matrix`
- [Nostr](/channels/nostr) — `@Root/nostr`
- [Zalo](/channels/zalo) — `@Root/zalo`
- [Microsoft Teams](/channels/msteams) — `@Root/msteams`
...
- `<workspace>/.Root/extensions/*.ts`
- `<workspace>/.Root/extensions/*/index.ts`

3. Global extensions

- `~/.Root/extensions/*.ts`
- `~/.Root/extensions/*/index.ts`

4. Bundled extensions (shipped with Root, **disabled by default**)
...
A plugin directory may include a `package.json` with `Root.extensions`:
...
Channel plugins can advertise onboarding metadata via `Root.channel` and
install hints via `Root.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@Root/nextcloud-talk",
  "Root": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
- `~/.Root/mpm/catalog.json`
- `~/.Root/plugins/catalog.json`

Or point `Root_PLUGIN_CATALOG_PATHS` (or `Root_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "Root": { "channel": {...}, "install": {...} } } ] }`.
...
Root augments `uiHints` at runtime based on discovered plugins:
...
Plugins may also register their own top‑level commands (example: `Root voicecall`).
...
```
import { registerPluginHooksFromDir } from "Root/plugin-sdk";
...
API-key setup inside Root (no external scripts needed).
...
- Main package: `Root` (this repo)
- Plugins: separate npm packages under `@Root/*` (example: `@Root/voice-call`)
...
- CLI: `Root voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `Root.extensions` points at the built entrypoint (`dist/index.js`).
