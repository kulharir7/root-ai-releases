...
Root ships two lightweight web tools:
...
environment. For a gateway install, put it in `~/.Root/.env` (or your
service environment). See [Env vars](/help/faq#how-does-Root-load-environment-variables).
...
environment. For a gateway install, put it in `~/.Root/.env`.

If no base URL is set, Root chooses a default based on the API key source:
