...
4. Users browse, download, and install skills in Root.
...
## How it fits into Root

By default, the CLI installs skills into `./skills` under your current working directory. If a Root workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). Root loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.Root/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Root how to perform a
...
Interested in becoming a moderator? Ask in the Root Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Root workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/Root/skills` and `~/.Root/skills`). This is designed to find older skill installs without extra flags.
