...
- **gateway host** → `Root` process on the gateway machine
...
`~/.Root/exec-approvals.json` directly.

CLI: `Root approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
