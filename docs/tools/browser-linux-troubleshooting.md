...
Root's browser control server fails to launch Chrome/Brave/Edge/Chromium with the error:
...
Enable with: `systemctl --user enable --now Root-browser.service`
...
You’re using the `chrome` profile (extension relay). It expects the Root
...
- Local `Root` profiles auto-assign `cdpPort`/`cdpUrl`; only set those for remote CDP.
