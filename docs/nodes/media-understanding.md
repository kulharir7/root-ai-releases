...
- If `<capability>.enabled: true` but no models are configured, Root tries the
  **active reply model** when its provider supports the capability.

### Auto-detect media understanding (default)

If `tools.media.<capability>.enabled` is **not** set to `false` and you haven’t
configured models, Root auto-detects in this order and **stops at the first
working option**:
...
## Provider support matrix (Root integrations)
