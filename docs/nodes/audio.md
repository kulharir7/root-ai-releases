...
- **Media understanding (audio)**: If audio understanding is enabled (or auto‑detected), Root:
...
Root auto-detects in this order and stops at the first working option:
...
When `requireMention: true` is set for a group chat, Root now transcribes audio **before** checking for mentions. This allows voice notes to be processed even when they contain mentions.

**How it works:**

1. If a voice message has no text body and the group requires mentions, Root performs a "preflight" transcription.
