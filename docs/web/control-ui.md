...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/Root`)
...
you revoke it with `Root devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.Root/Root.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Root
...
Root **blocks** Control UI connections without device identity.
