...
read_when:
  - Running Root behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Root
...
3. Root checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Root extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://Root.example.com
    to: http://Root-gateway:18789
    policy:
      - allow:
          or:
            - email:
