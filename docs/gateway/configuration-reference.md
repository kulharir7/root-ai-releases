...
description: "Complete field-by-field reference for ~/.Root/Root.json"
---
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["Root-dm"] },
      guilds: {
        "123456789012345678": {
...
Root spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@Root"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `Root.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Root auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Root assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.Root/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.Root/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@Root"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:Root`   | All built-in tools (excludes provider plugins)                                           |
...
- Override agent config root with `Root_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- Loaded from `~/.Root/extensions`, `<workspace>/.Root/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or Root_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `Root_GATEWAY_PORT` > `gateway.port` > `18789`.
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.Root/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-Root-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `Root_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or Root_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__Root__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__Root__/a2ui/`
...
- Also serves A2UI at `/__Root__/a2ui/`.
...
- Hostname defaults to `Root`. Override with `Root_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.Root/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `Root dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.Root/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.Root/credentials/oauth.json`.
...
- Default log file: `/tmp/Root/Root-YYYY-MM-DD.log`.
