...
- `funnel`: Public HTTPS via `tailscale funnel`. Root requires a shared password.
...
- `token` (default when `Root_GATEWAY_TOKEN` is set)
- `password` (shared secret via `Root_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Root verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Root only treats a request as Serve when it arrives from loopback with
...
Prefer `Root_GATEWAY_PASSWORD` over committing a password to disk.
