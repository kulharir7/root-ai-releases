...
<Tabs>
  ...
  <Tab title="Direct edit">
    Edit `~/.Root/Root.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Root only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>
...
The Gateway watches `~/.Root/Root.json` and applies changes automatically — no manual restart needed for most settings.
...
Root reads env vars from the parent process plus:
...
Env var equivalent: `Root_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${Root_GATEWAY_TOKEN}" } },
