...
- Extra workspace dir detection (`~/Root`).
...
- Rewrite `~/.Root/Root.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.Root/sessions/` to `~/.Root/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.Root/agent/` to `~/.Root/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.Root/credentials/*.json` (except `oauth.json`)
  - to `~/.Root/credentials/whatsapp/<accountId>/...` (default account id: `default`)
...
- **Multiple state dirs**: warns when multiple `~/.Root` folders exist across
  home directories or when `Root_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.Root/Root.json` is
