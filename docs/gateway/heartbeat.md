...
- During heartbeat runs, Root treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `Korvus sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Root skips the heartbeat run entirely (no model call).
...
```bash
Root system event --text "Check for urgent follow-ups" --mode now
```
