...
- Service types:
  - `_Root-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `Root` entrypoint or binary)
...
- `Root_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.Root/Root.json` controls the Gateway bind mode.
- `Root_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `Root_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `Root_CLI_PATH` overrides the advertised CLI path.
