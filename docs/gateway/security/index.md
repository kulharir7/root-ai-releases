...
- Tighten local perms (`~/.Root` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).
...
If you run `--deep`, Root also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.Root/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.Root/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.Root/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.Root/credentials/oauth.json`
...
gateway:
  ...
  auth:
    mode: password
    password: ${Root_GATEWAY_PASSWORD}
```
...
Root stores session transcripts on disk under `~/.Root/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.Root` (see the audit section below). If you need
...
Root pairing approve <channel> <code>
```
...
By default, Root routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
- “Paste the full contents of ~/.Root or your logs.”
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `Root_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__Root__/canvas/` and `/__Root__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_Root-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `Root_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `Root_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `Root_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Root
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Root verifies the identity by resolving the
...
Assume anything under `~/.Root/` (or `$Root_STATE_DIR/`) may contain secrets or private data:

- `Root.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@Root", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.Root/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `Root` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Root clients can connect.
...
1. Rotate Gateway auth (`gateway.auth.token` / `Root_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/Root/Root-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.Root/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Root version
...
Found a vulnerability in Root? Please report responsibly:

1. Email: [security@Root.ai](mailto:security@Root.ai)
