...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `Root_CONFIG_PATH` when set).
...
- Auth is required by default (`gateway.auth.token` / `gateway.auth.password`, or `Root_GATEWAY_TOKEN` / `Root_GATEWAY_PASSWORD`).

### Port and bind precedence

| Setting      | Resolution order                                              |
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `Root_GATEWAY_PORT` → `gateway.port` → `18789` |
...
- Unique `gateway.port`
- Unique `Root_CONFIG_PATH`
- Unique `Root_STATE_DIR`
