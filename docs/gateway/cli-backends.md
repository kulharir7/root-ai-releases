...
- If the primary provider fails (auth, rate limits, timeouts), Root will
...
2. **Builds a system prompt** using the same Root prompt + workspace context.
...
Root ships a default for `claude-cli`:
...
Root also ships a default for `codex-cli`:
...
- **No Root tools** (the CLI backend never receives tool calls). Some CLIs
