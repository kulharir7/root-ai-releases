...
<Steps>
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `Root pairing approve <channel> <code>` or use allowlists.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.Root/.env` so launchd can read it.
...
- OAuth credentials: `~/.Root/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.Root/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.Root/credentials/oauth.json` (or `$Root_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
