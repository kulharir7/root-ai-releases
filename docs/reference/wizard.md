...
<Steps>
  <Step title="Existing config detection">
    - If `~/.Root/Root.json` exists, choose **Keep / Modify / Reset**.
  ...
  <Step title="Model/Auth">
    ...
    - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.Root/.env` so launchd can read it.
    ...
    - OAuth credentials live in `~/.Root/credentials/oauth.json`; auth profiles live in `~/.Root/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.Root/credentials/oauth.json` (or `$Root_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.Root/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `Root pairing approve <channel> <code>` or use allowlists.
  </Step>
...
- Stores it under `~/.Root/tools/signal-cli/<version>/`.
...
Typical fields in `~/.Root/Root.json`:
