...
This document explains how Root manages sessions end-to-end:
...
Root persists sessions in two layers:
...
- Transcripts: `~/.Root/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Root resolves these via `src/config/sessions.ts`.
...
Root intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Root consumes the events, but Pi decides when to compact).
...
Root also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Root bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Root leaves it alone.
...
- Root strips/suppresses this in the delivery layer.

As of `2026.1.10`, Root also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Pi also exposes a `session_before_compact` hook in the extension API, but Root’s
