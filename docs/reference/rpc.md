...
Root integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Root owns lifecycle when `channels.signal.autoStart=true`.
...
- Root spawns `imsg rpc` as a child process (legacy iMessage integration).
