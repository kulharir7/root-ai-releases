...
Copilot API tokens when Root runs. This is the **default** and simplest path
because it does not require VS Code.

### 2) Copilot Proxy plugin (`copilot-proxy`)

Use the **Copilot Proxy** VS Code extension as a local bridge. Root talks to
...
- The login stores a GitHub token in the auth profile store and exchanges it for a
  Copilot API token when Root runs.
