---
summary: "Use Venice AI privacy-focused models in Root"
read_when:
  - You want privacy-focused inference in Root
...
```bash
Root chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Root shows all available Venice models. Pick based on your needs:
...
Root automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
Root chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
Root chat --model venice/claude-opus-45

# Use uncensored model
Root chat --model venice/venice-uncensored

# Use vision model with image
Root chat --model venice/qwen3-vl-235b-a22b

# Use coding model
Root chat --model venice/qwen3-coder-480b-a35b-instruct
```
