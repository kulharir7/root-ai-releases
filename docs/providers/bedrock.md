---
summary: "Use Amazon Bedrock (Converse API) models with Root"
read_when:
  - You want to use Amazon Bedrock models with Root
...
When running Root on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, Root's credential detection currently only checks for environment
...
- Root surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
