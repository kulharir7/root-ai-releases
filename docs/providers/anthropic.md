---
summary: "Use Anthropic Claude via API keys or setup-token in Root"
read_when:
  - You want to use Anthropic models in Root
...
In Root you can authenticate with an API key or a **setup-token**.
...
When using Anthropic API Key authentication, Root automatically applies `cacheRetention: "short"` (5-minute cache) for all Anthropic models. You can override this by explicitly setting `cacheRetention` in your config.
...
Root includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
