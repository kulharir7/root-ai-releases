---
summary: "Use Synthetic's Anthropic-compatible API in Root"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Root registers it as the
...
Note: Root's Anthropic client appends `/v1` to the base URL, so use
