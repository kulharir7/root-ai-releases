...
Edge TTS does **not** require an API key. If no API keys are found, Root defaults
...
TTS config lives under `messages.tts` in `Root.json`.
...
- If `provider` is **unset**, Root prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.Root/settings/tts.json`, override with `Root_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Root retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Root registers
