...
```
  ...
  <Tab title="Remote Mac over SSH">
    Root only requires a stdio-compatible `cliPath`, so you can point `cliPath` at a wrapper script that SSHes to a remote Mac and runs `imsg`.
...
```
    ...
    If `remoteHost` is not set, Root attempts to auto-detect it by parsing the SSH wrapper script.
...
- Full Disk Access is required for the process context running Root/`imsg` (Messages DB access).
...
<Tabs>
  ...
  <Tab title="Sessions and deterministic replies">
    ...
    If that `chat_id` is explicitly configured under `channels.imessage.groups`, Root treats it as group traffic (group gating + group session isolation).
...
```
  ...
  <Accordion title="DMs are ignored">
    ...
    - pairing approvals (`Root pairing list imessage`)
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs Root/`imsg`.
