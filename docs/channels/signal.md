...
- Root installed on your server (Linux flow below tested on Ubuntu 24).
...
2. Link a bot account:
   - `signal-cli link -n "Root"` then scan the QR in Signal.
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `Root pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Root at it:
...
This skips auto-spawn and the startup wait inside Root. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `Root pairing list signal`
  - `Root pairing approve signal <CODE>`
...
- **Typing indicators**: Root sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Root forwards read receipts for allowed DMs.
...
Root pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/Root/Root-$(date +%Y-%m-%d).log" | tail -20
```
