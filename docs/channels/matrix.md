...
Matrix is an open, decentralized messaging protocol. Root connects as a Matrix **user**
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Root calls the same
     login endpoint, stores the access token in `~/.Root/credentials/matrix/credentials.json`,
...
`~/.Root/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Root resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
