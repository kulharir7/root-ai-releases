...
1. Enable IRC config in `~/.Root/Root.json`.
2. Set at least:
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Root defaults to **mention-gating** in group contexts.
