...
2. Create a **Service Account**:
   ...
   - Name it whatever you want (e.g., `Root-chat`).
...
4. Store the downloaded JSON file on your gateway host (e.g., `~/.Root/googlechat-service-account.json`).
5. Create a Google Chat app in the [Google Cloud Console Chat Configuration](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat):
   - Fill in the **Application info**:
     - **App name**: (e.g. `Root`)
     - **Avatar URL**: (e.g. `https://Root.ai/logo.png`)
...
Google Chat webhooks require a public HTTPS endpoint. For security, **only expose the `/googlechat` path** to the internet. Keep the Root dashboard and other sensitive endpoints on your private network.
...
With this config, any request to `your-domain.com/` will be ignored or returned as 404, while `your-domain.com/googlechat` is safely routed to Root.
...
2. Root verifies the token against the configured `audienceType` + `audience`:
...
4. DM access is pairing by default. Unknown senders receive a pairing code; approve with:
   - `Root pairing approve googlechat <code>`
