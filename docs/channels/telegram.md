...
- DM messages can carry `message_thread_id`; Root routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Root retries as plain text.
...
```
    ...
    - `~/.Root/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Root enqueues system events like:
