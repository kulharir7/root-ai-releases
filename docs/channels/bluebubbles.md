...
- Root talks to it through its REST API (`GET /api/v1/ping`, `POST /message/text`, `POST /chat/:id/*`).
...
- **Typing indicators**: Root sends typing start events; BlueBubbles clears typing automatically on send or timeout (manual stop via DELETE is unreliable).
...
Root may surface _short_ message IDs (e.g., `1`, `2`) to save tokens.
...
- Pairing codes expire after one hour; use `Root pairing list bluebubbles` and `Root pairing approve bluebubbles <code>`.
