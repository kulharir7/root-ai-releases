...
Root routes replies **back to the channel where a message came from**. The
...
Broadcast groups let you run **multiple agents** for the same peer **when Root would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.Root/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.Root`):
