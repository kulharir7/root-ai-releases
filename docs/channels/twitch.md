...
Twitch chat support via IRC connection. Root connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `Root_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "Root", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use Root_TWITCH_ACCESS_TOKEN env var)
...
```bash
Root_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
