...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up Root?](#whats-the-recommended-way-to-install-and-set-up-Root)
  ...
  - [I can't access docs.Root.ai (SSL error). What now?](#i-cant-access-docsRootai-ssl-error-what-now)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can Root run tasks on a schedule or continuously in the background?](#can-Root-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with Root saved locally?](#is-all-data-used-with-Root-saved-locally)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two Root instances talk to each other (local + VPS)?](#can-two-Root-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does Root load environment variables?](#how-does-Root-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of Root instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-Root-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset Root but keep it installed?](#how-do-i-completely-reset-Root-but-keep-it-installed)
  ...
  - [Why doesn't Root reply in a group?](#why-doesnt-Root-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do Root, Flawd, and Krill use for models?](#what-do-Root-flawd-and-krill-use-for-models)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/Root/Root-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   Root health --json
   Root health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://Root.ai/install.sh | bash -s -- --install-method git
```

This installs Root **from a git checkout**, so the agent can read the code + docs and
...
[https://github.com/Root/Root/issues](https://github.com/Root/Root/issues)
[https://github.com/Root/Root/pulls](https://github.com/Root/Root/pulls)
...
### What's the recommended way to install and set up Root
...
```bash
git clone https://github.com/Root/Root.git
cd Root
pnpm install
pnpm build
...
under `~/.Root/` (for example `~/.Root/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-Root-store-its-data),
...
[https://github.com/Root/Root/blob/main/CHANGELOG.md](https://github.com/Root/Root/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docs.Root.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.Root.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.Root.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsRootai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/Root/Root/tree/main/docs](https://github.com/Root/Root/tree/main/docs)
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://Root.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://Root.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://Root.ai/install.ps1](https://Root.ai/install.ps1)
...
```bash
curl -fsSL https://Root.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://Root.ai/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://Root.ai/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://Root.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or Root not recognized
...
Yes - via **setup-token**. Root no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Yes. Root fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `Root.json`.
...
Usually no. Root needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but Root authorization uses numeric IDs only.
...
### Can multiple people use one WhatsApp number with different Root instances
...
This **does not delete your data** - it only changes the Root code install. Your state
(`~/.Root`) and workspace (`~/.Root/workspace`) stay untouched.
...
**Root-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).
...
### Can Root help with lead gen outreach ads and blogs for a SaaS
...
Showcase: [https://Root.ai/showcase](https://Root.ai/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.Root/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.Root/Root.json`). Precedence is `<workspace>/skills` > `~/.Root/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.Root/Root.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.Root/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Root treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `Root_SKIP_CRON` is not set.
...
### Can Root run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.Root.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
...
3. Override the skill metadata (workspace or `~/.Root/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "Root": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured Root workspace); Root treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.Root/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- Persist `/home/node` with `Root_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `Root_DOCKER_APT_PACKAGES`.
...
Root also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, Root auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, Root
prefers `local`.
...
### Is all data used with Root saved locally

No - **Root's state is local**, but **external services still see what you send them**.
...
### Where does Root store its data

Everything lives under `$Root_STATE_DIR` (default: `~/.Root`):

| Path                                                            | Purpose                                                      |
| --------------------------------------------------------------- | ------------------------------------------------------------ |
| `$Root_STATE_DIR/Root.json`                             | Main config (JSON5)                                          |
| `$Root_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use) |
| `$Root_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys)                             |
| `$Root_STATE_DIR/agents/<agentId>/agent/auth.json`          | Runtime auth cache (managed automatically)                   |
| `$Root_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)      |
| `$Root_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                        |
...
These files live in the **agent workspace**, not `~/.Root`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.Root`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.Root/skills`).

Default workspace is `~/.Root/workspace`, configurable via:
...
Do **not** commit anything under `~/.Root` (credentials, sessions, tokens).
...
`workspace` to the repo root. The Root repo is just source code; keep the
...
Root reads an optional **JSON5** config from `$Root_CONFIG_PATH` (default: `~/.Root/Root.json`):

```
$Root_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.Root/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `Root_GATEWAY_TOKEN`).
...
- Daemons read env vars from `~/.Root/.env` (or the service environment).
...
### Can two Root instances talk to each other local VPS
...
### How does Root load environment variables

Root reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.Root/.env` (aka `$Root_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.Root/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`Root_LOAD_SHELL_ENV=1`, `Root_SHELL_ENV_TIMEOUT_MS=15000`.
...
### Is there a way to make a team of Root instances one CEO and many agents
...
### How do I completely reset Root but keep it installed
...
```bash
Root reset --scope full --yes --non-interactive
```
...
headers like `# Heading`), Root skips the heartbeat run to save API calls.
...
```bash
Root directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesnt Root reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.Root/agents/<agentId>/sessions/`.
...
Root's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Root currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
### What do Root, Flawd, and Krill use for models

- **Root + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
...
Yes. Root ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.Root/agents/<agentId>/agent/auth-profiles.json
```
...
Fix: Root now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
- The Control UI stores the token in browser localStorage key `Root.control.settings.v1`.
...
- `Root_CONFIG_PATH` (per-instance config)
- `Root_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `Root --profile <name> …` per instance (auto-creates `~/.Root-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `Root --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.Root.*`, `Root-gateway-<profile>.service`, `Root Gateway (<profile>)`).
...
```
Root tui --url ws://<host>:18789 --token <token>
```
...
- macOS: `$Root_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.Root/logs/...`; profiles use `~/.Root-<profile>/logs/...`)
- Linux: `journalctl --user -u Root-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "Root Gateway (<profile>)" /V /FO LIST`
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Root assistant setup](/start/Root) and [Agent send](/tools/agent-send).
...
- Default behavior on DM-capable channels is **pairing**:
  ...
  - Approve with: `Root pairing approve <channel> <code>`
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Root only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
Root pairing approve whatsapp <code>
```

List pending requests:

```bash
Root pairing list whatsapp
```
...
Root blocks **cross-provider** messaging by default. If a tool call is bound
