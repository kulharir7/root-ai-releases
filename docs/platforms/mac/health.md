...
- You can still use the CLI flow in [Gateway health](/gateway/health) (`Root status`, `Root status --deep`, `Root health --json`) and tail `/tmp/Root/Root-*.log` for `web-heartbeat` / `web-reconnect`.
