---
summary: "Root on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Root on a Raspberry Pi
  - Running Root on ARM devices
...
Run a persistent, always-on Root Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Root features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled Root
...
```bash
# Check logs
journalctl -u Root --no-pager -n 100
