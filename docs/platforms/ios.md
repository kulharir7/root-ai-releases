...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `Root.internal.`), **or**
...
The Gateway advertises `_Root-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `Root.internal.`) and Tailscale split DNS.
...
- The Gateway canvas host serves `/__Root__/canvas/` and `/__Root__/a2ui/`.
