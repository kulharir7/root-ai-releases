---
summary: "Root macOS companion app (menu bar + gateway broker)"
read_when:
...
- Installs the global CLI (`Root`) via npm/pnpm on request (bun not recommended for the Gateway runtime).
...
(or `bot.molt.<profile>` when using `--profile`/`Root_PROFILE`; legacy `com.Root.*` still unloads).
...
The app registers the `Root://` URL scheme for local actions.
...
```bash
open 'Root://agent?message=Hello%20from%20deep%20link'
```
