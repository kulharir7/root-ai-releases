...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Root
...
Running Root via Node with `tsx` fails at startup with:

```
[Root] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `Korvus status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch Root.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node Root.mjs status` works on Node 25.
