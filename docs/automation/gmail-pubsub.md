...
under `~/.Root/hooks/transforms` (see [Webhooks](/automation/webhook)).
...
- Set `Root_SKIP_GMAIL_WATCHER=1` to opt out (useful if you run the daemon yourself).
...
```bash
gog gmail watch status --account Root@gmail.com
gog gmail history --account Root@gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account Root@gmail.com
