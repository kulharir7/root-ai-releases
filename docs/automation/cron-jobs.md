...
- Jobs persist under `~/.Root/cron/` so restarts don’t lose schedules.
...
If `delivery` is omitted for isolated jobs, Root defaults to `announce`.
...
- Job store: `~/.Root/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.Root/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```bash
Root system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`Root system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `Root_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Root applies exponential retry backoff for recurring jobs after consecutive errors:
