...
Deploy Root on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/Root` — Control UI
...
- `Root_STATE_DIR=/data/.Root` (recommended)
- `Root_WORKSPACE_DIR=/data/workspace` (recommended)
- `Root_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Root state + workspace so you can migrate to another host without losing config or memory.
