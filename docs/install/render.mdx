...
Deploy Root on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/Root/Root)
...
repository configures everything needed to run Root:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: Root_STATE_DIR
        value: /data/.Root
      - key: Root_WORKSPACE_DIR
        value: /data/workspace
      - key: Root_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/Root`.
...
If you use the original Root repository, Render will not auto-deploy your Root. To update it, run a manual Blueprint sync from the dashboard.
...
For Root, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Root host.
