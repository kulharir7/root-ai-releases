---
summary: "Updating Root safely (global install or source), plus rollback strategy"
read_when:
  - Updating Root
...
- For **global installs**, the script uses `npm install -g Root@latest` under the hood.
...
- Detect and migrate older gateway services (launchd/systemd; legacy schtasks) to current Root services.
...
```bash
npm i -g Root@<version>
```

```bash
pnpm add -g Root@<version>
```

Tip: to see the current published version, run `npm view Root version`.
