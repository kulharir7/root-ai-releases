...
Root ships three update channels:
...
When you **explicitly** switch channels with `--channel`, Root also aligns
the install method:

- `dev` ensures a git checkout (default `~/Root`, override with `Root_GIT_DIR`),
